...
const Pointer = (props: IPointerProps) => {
  ...
  const { pointer, svg, $svg, data, settings, setPointer, selectedPointerId } =
    props;
  ...
  }, [data, pointer.angleDeg, svg.startAngleDeg, svg.endAngleDeg]);

  useEffect(() => {
    const angleRad = convertRange(
  ...
  const onValueChange = useCallback(
    (
      ...
      if (!isAngleInArc(svg.startAngleDeg, svg.endAngleDeg, degrees)) {
  ...
  return (
    <>
      {center && (
        <g
          ref={pointerRef}
          transform={`translate(${center[0] - radius / 2}, ${
            center[1] - radius / 2
          })`}
          role="slider"
          ...
          data-type="pointer"
          className={`mz-round-slider-pointer ${
            pointer.disabled ? "mz-round-slider-pointer-disabled" : ""
          }`}
        ...
        >
          ...
          {settings.pointerSVG && <g>{settings.pointerSVG}</g>}
        </g>
      )}
    </>
  );
